...
// make sure the picker intially returns NoSubConnAvailable before resolver has
// discovered any servers and updated the picker's state with available sub connections.
...
func TestPickerConsumerFromFollowers(t *testing.T) {
	picker, subConns := setupTest()
	info := balancer.PickInfo{
